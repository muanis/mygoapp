...
function weighCats(cats chan []byte, weights chan int64, wg *sync.WaitGroup) {
	defer wg.Done()
...
function weighCat( cat []byte) (int64, error) {
	resp, err := http.Post("http://localhost:8080/weighCats", "application/json", bytes.NewReader(cat))
	if err != nil {
		return 0, err
	}
